...
option_list <- list( 
  ...
  make_option( "--database", type="character", default="kegg",
              help="Database to get gmt from. Supported: kegg, wikipathways, reactome, panglaodb. (Default: kegg)"),
  make_option(c("-o", "--out"), type="character", default="pathways.gmt",
...
get_gmt <- function(species = "hsa", db ="kegg"){
  ...
  if (db=="kegg"|| db=="wikipathways") {
    ...
    ncbi <- read.csv(gzfile(temp, ncbi.filename), header = T, sep = "\t", comment.char = "", quote = "", stringsAsFactors = F)
    closeAllConnections() 
  ...
  if (db=="wikipathways"){
    ...
    wikipathways.gmt.url <- "http://data.wikipathways.org/current/gmt/"
    doc <- htmlParse(wikipathways.gmt.url)
    links <- xpathSApply(doc, "//a/@href")
    free(doc)
    wanted <- strsplit(links[grepl(wikipaths.suffix.filename, links)][1], split = "./")[[1]][2]
  ...
  if (db=="panglaodb"){
    ...
    #path_to_tsv="/Users/kris.g/maConstruction/scripts/gmt/src/"
  ...
  if (db=="go"){
    ...
    GOurl <- "ftp://ftp.ncbi.nlm.nih.gov/gene/DATA/gene2go.gz"
    temp <- tempfile()
    download.file(GOurl, temp)
    filepath.unzpd <- "/Users/kris.g/Downloads/gene2go"
    test2 <- (gzfile(temp2))
    test1 <- read.table(filepath.unzpd)
    closeAllConnections() 
    print("Reactome annotations successfully downloaded.")
    print("Transforming to .gmt file...")
  }
...
write.gmt3 <- function(gmtlist, filename){
  ...
  for (i in seq_along(gmtlist)) {
    cat(gsname[i], strsplit(filename,".gmt")[[1]], gmtlist[[i]], file = filename, 
        append = TRUE, sep = "\t")
