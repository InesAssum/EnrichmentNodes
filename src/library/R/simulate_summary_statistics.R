...
option_list = list(
  make_option("--label", type="character", default=NULL,
              help="Label for the simulated data. Date stamp if empty."),
  make_option("--out1", type="character", default="simSTATS.tar.gz",
              help="Filename of the summary statistics data results."),
  make_option("--out2", type="character", default="simQC.tar.gz",
              help="Filename of the QC summary files."),
  make_option("--outtxt", type="character", default="parameter_file.txt",
              help="Textfile with simulation parameter run information."),
  make_option( "--seed", type="double", default=NA,
               help="Run simulations with a specific seed. Randomly set otherwise."),
  make_option( "--Niter", type="double", default=3,
               help="Number of replicates/iterations/scenarios to sample."),
  make_option("--gmt", type="character", default=NULL,
              help="filepath to gmt pathway definitions."),
  make_option("--scenario", type="character", default="shared",
              help="Shared or independent pathway activations across omics."),
  make_option("--nactive", default=6, type = "integer",
              help="Number of shared pathways to be active. Set to 0 for independent scenario."),
  make_option("--nactive1", type="double", default=0,
              help="Number of gene sets active in the first species. Ignored for shared pathways." ),
  make_option("--nactive2", type="double", default=0,
              help="Number of gene sets active in the second species. Ignored for shared pathways." ),
  make_option("--cov", type="double", default=0.3,
              help="Coverage of the second species - fraction of genes included in pathway annotations (default: 0.3)." ),
  make_option("--rho", type="double", default=0.3,
              help="Correlation between omics (default: 0.3)."),
  make_option( "--miss", type="double", default=3,
               help="Do you want to prefilter gene sets for a certain size? All gene sets are considered, if this is set to 0. Otherwise, only gene sets bigger than this threshold will be considered."),
  make_option("--sdbg", type="double", default=2,
              help="Standard deviation of the background normal distribution.")
  # make_option( "--signal", type="character", default="default",
  #              help="Standard deviation for the signal normal distribution.")
); 
...
nactive   <- opt$nactive
nactive1  <- opt$nactive1
nactive2  <- opt$nactive2
coverage  <- opt$cov
rho       <- opt$rho
miss      <- opt$miss
sd.bg     <- opt$sdbg
sd.signal <- "1.5-alpha" #opt$sdsignal

#gmt.path <- "~/data/pathways/c2.cp.kegg.v7.5.1.symbols.gmt"
myseed <- ifelse(is.na(seed), NA, as.integer(seed))
#' #############################################################################
...
simulate_summary_stats <- function(label, outdir,
                                   seed=NA, error.rates=NA,
                                   iterations=1:100, 
                                   gmt,
                                   gmt.path=NA,
                                   coverage=0.3,
                                   scenario="shared", nactive = 6,
                                   rho=0.3, filter.miss=5,
                                   sd.bg=2,
                                   sd.signal="1.5-alpha"){
...
if(nactive==0 & scenario=="independent" & nactive1>0 & nactive2>0){
  nactive <- list(spec1=nactive1, spec2=nactive2)
  print("Independent scenario of pathway activations.")
}else if(nactive>0 & scenario=="shared"){
  nactive[["shared"]] <- nactive
  print("Shared scenario of pathway activations.")
}else{
  print("Invalid simulation scenario.")
}
...
if(is.na(as.numeric(sd.signal))){
  sd.signal <- sd.signal
}else{
  sd.signal <- as.numeric(sd.signal)
}
...
print(head(error.rates))

outdir <- tempdir()

print("Start simulations")
res <- simulate_summary_stats(label=label, outdir=outdir,
                       seed = myseed,
                       iterations=1:Niter, error.rates=error.rates,
                       gmt=gmt, gmt.path=gmt.path,
                       coverage=coverage,
                       scenario=scenario, nactive = nactive,
                       rho=rho, filter.miss=miss,
                       sd.bg=sd.bg,
                       sd.signal=sd.signal)

write.table(data.frame(t(res$parameters)),
            file=file,
            row.names = T, col.names = F, quote = F, sep = "\t")

setwd(paste0(outdir, "/", label, "/"))
tar("sum_stats.tar.gz", "sum_stats/",
    compression = 'gzip', tar="tar")
file.copy("sum_stats.tar.gz", tarSTATS)

tar("QC_stats.tar.gz", "QC_stats/",
    compression = 'gzip', tar="tar")
file.copy("QC_stats.tar.gz", tarQC)
